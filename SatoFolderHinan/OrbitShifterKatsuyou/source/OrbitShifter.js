...
export class OrbitShifter {
    ...
    constructor(Frame, Ease, x1, y1, x2, y2) {
        ...
        this.speed = this.distance / this.frame;//1px進むのに必要なフレーム数
        ...
        if (this.sinCosDetector == "sin") {
            this.tween = new TweenAnime.FrameTween([0, 0], [2 * this.roundNum * Math.PI * this.direction, 0], this.frame, this.ease);
        }
        else if(this.sinCosDetector == "cos"){
            this.tween = new TweenAnime.FrameTween([0.5* Math.PI, 0], [2 * this.roundNum * Math.PI * this.direction + 0.5 * Math.PI, 0], this.frame, this.ease);
        }
    }

    calcSinCosDetector(x1, y1, x2, y2) {
        let sinCosDetector;
        if (Math.abs(x1 - x2) < Math.abs(y1 - y2)) {sinCosDetector = "cos";}
        if(Math.abs(x1 - x2) >= Math.abs(y1 - y2)){sinCosDetector = "sin";}
    ...
    setRoundNum(Speed) {
        let num;
        let randomRange
        randomRange = Speed * 50;
        num = Math.ceil(Math.random(0, randomRange));
        num = 1;
        return num;
    }
