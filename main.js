import p5 from "p5";
import { loadElement } from "./utils/canvas/loadElement";
import { P5CanvasManager } from "./utils/canvas/p5CanvasManager";
...
const sketch = (p5) => {
    ...
    p5.setup = () => {
        ...
        for (let i = 0; i < 10; i++) {
            const testToMove = new ToMove(p5, 300 + i * 20, 300, 300, 300 + i * 20, 450, p5.random(0, 10), TweenAnime.Easing.easeOutSine);
            const test = new Ball(p5, TweenAnime.Easing.easeInOutQuint, 80, testToMove);
            tests.push(test);
        }
    };
    p5.draw = () => {
...
class Ball {
    ...
    update() {
        ...
        console.log(this.tween.frame);
        console.log(this.frameCurrent);
        this.tween.update();
        this.position = {
            x: this.tween.getValues()[0],
            y: this.tween.getValues()[1],
        };
